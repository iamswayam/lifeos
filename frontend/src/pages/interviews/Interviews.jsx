...
const emptyForm = { company_name:'', role:'', package_offered:'', job_url:'', hr_name:'', hr_contact:'', round_number:1, round_type:'screening', mode:'online', platform:'meet', location:'', scheduled_at:getDefaultDateTime(), follow_up_date:'', prep_notes:'', status:'scheduled', result:'waiting' }
...
const RESULT_OPTIONS = ['waiting','selected','rejected','on_hold']
...
export default function Interviews() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) { await updateInterview(editing, payload); toast.success('Updated!') }
      else         { await createInterview(payload);          toast.success('Interview added!') }
      setShowForm(false); setForm(emptyForm); setEditing(null); load()
    } catch (err) {
      const errorMsg = err.response?.data ? Object.values(err.response.data)?.[0]?.[0] || JSON.stringify(err.response.data) : 'Failed to save'
  ...
  const handleFeedback = async () => {
    try {
      // leave status unchanged except mark completed when result isn't 'waiting'
      const payload = { feedback: feedback.text, result: feedback.result }
      if (feedback.result !== 'waiting') payload.status = 'completed'
      await addFeedback(feedback.id, payload)
        ...
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'14px', padding:'24px', marginBottom:'20px' }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'14px' }}>
              {inp('Company Name *', 'company_name', 'text', 'Google', true)}
              {inp('Role *', 'role', 'text', 'Backend Engineer', true)}
              ...
              {inp('Scheduled At *', 'scheduled_at', 'datetime-local', '', true)}
              ...
              {sel('Result', 'result', RESULT_OPTIONS)}
            </div>
      ...
      {feedback.id && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:999 }}>
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'16px', padding:'28px', width:'100%', maxWidth:'480px' }}>
            ...
            <div style={{ marginBottom:'14px' }}>
              <label style={{ display:'block', fontSize:'12px', fontWeight:500, color:'var(--muted)', marginBottom:'5px' }}>Status</label>
        ...
        <div style={{ display:'grid', gap:'12px' }}>
          ...
          {interviews.map(i => (
            ...
            >
              ...
              {i.feedback   && <div style={{ fontSize:'12px', color:'var(--muted)', padding:'10px 12px', background: resultBg[i.result] || 'var(--green-bg)', borderRadius:'8px', marginTop:'8px' }}>💬 {i.feedback}</div>}
            </div>
          ))}
        </div>
      )}
